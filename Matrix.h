...
public:
    ...
    string toString() // we have access to each item in matrix (both cost and indexes)
    {
        ...
        for (int i = 0; i < numColumn; i++)
        {
            ...
            {
                stringRep += string("(") + to_string(representation[i][j].getStateType().getX()) + string(", ") + to_string(representation[i][j].getStateType().getY()) /*+ string(") : cost: ")*/
        ...
        stringRep += "GOAL IS \n";
        stringRep += to_string(this->getGoalState().getStateType().getX());
        stringRep += to_string(this->getGoalState().getStateType().getY());
    ...
    vector<State<T> *> getAllPossibleStates(State<T> &overItCheck)
    {
        ...
        if (    (0 < xOverCheck) && (xOverCheck < numRows - 1) && 
        (0 < yOverCheck) && (yOverCheck < numColumn - 1)     ) // middle
        {
            State<T> *goLeft = &representation[xOverCheck][yOverCheck - 1];
            State<T> *goRight = &representation[xOverCheck][yOverCheck + 1];
            State<T> *goUp = &representation[xOverCheck - 1][yOverCheck];
    ...
    void setInitPos()
    {
        this->initPos = &representation[startX][startY];
    }
