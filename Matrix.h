...
public:
    ...
    string toString() // we have access to each item in matrix (both cost and indexes)
    {
        ...
        for (int i = 0; i < numColumn; i++)
        {
            ...
            {
                stringRep += string("(") + to_string(representation[i][j].getStateType().getX()) + string(", ") + to_string(representation[i][j].getStateType().getY()) /*+ string(") : cost: ")*/ + ")" + to_string(int(representation[i][j].getCost())) + string(", ");
            }
            stringRep += "\n";
