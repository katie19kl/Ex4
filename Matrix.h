    ...
    void fullFillFields(vector<string>* parsed) {
        ...
        if (parsed->back() == "end") {
            ...
            for (int i = 0; i < numRows; i++) {
                ...
                k = 0; //used for iterating over the chars in a string representation of a specific matrix line
                for (int j = 0; j < numColumn; j++) {
                    cout << "x value: " << i << ", y value: " << j << " cost: " << parsed->at(i)[k] << endl;
                    //T item = new T(i, j, parsed.at(i)[k]);
                    representation[j + numRows * i] = new State<T>(i, j, parsed->at(i)[k]);
                    k += 2; //jumping over comas
                }
            }
