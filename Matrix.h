        ...
        if (parsed->back() == "end") {
            ...
            for (int i = 0; i < numRows; i++) { //going over y values
                ...
                for (int j = 0; j < numColumn; j++) { //going over x values
                    ...
                    double actualCost;
                    string cost = parsed->at(i).substr(l, k - l);
                    stringstream costInDouble(cost);
                    costInDouble >> actualCost;

                    cout << "x value: " << j << ", y value: " << i << " cost: " << cost << endl;
                    //T item = new T(i, j, actualCost);
